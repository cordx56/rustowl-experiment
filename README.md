...
引数を取り返り値を返す関数は次のように書く。

```rust
fn print(s: String, n: i32) -> i32 {
    println!("{n}: {s}");
    return n + 1;
}
```

引数および返り値の型は記述しなければならない。
...
参照先の値を読んだり書き換えたりする場合には、参照の前に`*`をつける。
...
所有権を移動せずに同じ値を複製するには、`.clone()`メソッドを呼び出すことで値の複製ができる。
